...
const LCL_PROVIDER_FIELDS: &[FieldSpec] = &[
    FieldSpec {
        name: "kind",
        expected: FieldType::String,
    },
...
const LCL_LLAMASERVER_FIELDS: &[FieldSpec] = &[
    FieldSpec {
        name: "bin",
        expected: FieldType::String,
    },
...
pub fn validate_config_file(
    ...
    if let Some(lcl) = object.get("lcl").and_then(JsonValue::as_object) {
        result.merge(validate_object_keys(
            lcl,
            LCL_FIELDS,
            "lcl",
        ...
        if let Some(provider) = lcl.get("provider").and_then(JsonValue::as_object) {
            result.merge(validate_object_keys(
                provider,
                LCL_PROVIDER_FIELDS,
                "lcl.provider",
        ...
        if let Some(llama_server) = lcl.get("llamaServer").and_then(JsonValue::as_object) {
            result.merge(validate_object_keys(
                llama_server,
                LCL_LLAMASERVER_FIELDS,
                "lcl.llamaServer",
                source,
                &path_display,
            ));
...
mod tests {
    ...
    #[test]
    fn validates_nested_lcl_provider_keys() {
        let source = r#"{"lcl":{"provider":{"kind":"openai-compat","baseURL":"bad"}}}"#;
        let parsed = JsonValue::parse(source).expect("valid json");
        let object = parsed.as_object().expect("object");
        ...
        assert_eq!(result.errors.len(), 1);
        assert_eq!(result.errors[0].field, "lcl.provider.baseURL");
    }

    #[test]
    fn validates_nested_lcl_llama_server_types() {
        let source = r#"{"lcl":{"llamaServer":{"port":"8080"}}}"#;
        let parsed = JsonValue::parse(source).expect("valid json");
        let object = parsed.as_object().expect("object");
        ...
        assert_eq!(result.errors.len(), 1);
        assert_eq!(result.errors[0].field, "lcl.llamaServer.port");
        assert!(matches!(
            result.errors[0].kind,
            DiagnosticKind::WrongType {
                expected: "a number",
