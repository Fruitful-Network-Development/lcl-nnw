...
/// LCL-specific local runtime settings layered on top of upstream Claw config.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct LclConfig {
    provider: Option<LclProviderConfig>,
    llama_server: Option<LclLlamaServerConfig>,
}

/// Local provider transport overrides for `lcl`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LclProviderConfig {
    pub kind: String,
    pub base_url: Option<String>,
    pub api_key: Option<String>,
}

/// Local `llama-server` launch settings for `lcl`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LclLlamaServerConfig {
...
impl ConfigLoader {
    ...
    pub fn load(&self) -> Result<RuntimeConfig, ConfigError> {
        ...
        let feature_config = RuntimeFeatureConfig {
            ...
            lcl: parse_optional_lcl_config(&merged_value)?,
        };
...
impl RuntimeConfig {
    ...
    #[must_use]
    pub fn lcl(&self) -> &LclConfig {
        &self.feature_config.lcl
    }
}
...
impl LclConfig {
    #[must_use]
    pub fn provider(&self) -> Option<&LclProviderConfig> {
        self.provider.as_ref()
    }

    #[must_use]
    pub fn llama_server(&self) -> Option<&LclLlamaServerConfig> {
        self.llama_server.as_ref()
    }
}

impl LclProviderConfig {
    #[must_use]
    pub fn is_openai_compat(&self) -> bool {
        self.kind == "openai-compat"
    }
}
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("LCL_CONFIG_HOME")
...
fn parse_optional_lcl_config(root: &JsonValue) -> Result<LclConfig, ConfigError> {
    let Some(object) = root.as_object() else {
        return Ok(LclConfig::default());
    };
    let Some(value) = object.get("lcl") else {
        return Ok(LclConfig::default());
    };
    let lcl = expect_object(value, "merged settings.lcl")?;
    let provider = lcl
        .get("provider")
        .map(|provider| parse_lcl_provider_config(provider, "merged settings.lcl.provider"))
        .transpose()?;
    let llama_server = lcl
        .get("llamaServer")
        .map(|entry| parse_lcl_llama_server_config(entry, "merged settings.lcl.llamaServer"))
        .transpose()?;
    Ok(LclConfig {
        provider,
        llama_server,
...
fn parse_lcl_provider_config(
    value: &JsonValue,
    context: &str,
) -> Result<LclProviderConfig, ConfigError> {
    let object = expect_object(value, context)?;
    Ok(LclProviderConfig {
        kind: expect_string(object, "kind", context)?.to_string(),
        base_url: optional_string(object, "baseUrl", context)?.map(str::to_string),
        api_key: optional_string(object, "apiKey", context)?.map(str::to_string),
    })
}

fn parse_lcl_llama_server_config(
    value: &JsonValue,
    context: &str,
) -> Result<LclLlamaServerConfig, ConfigError> {
    let object = expect_object(value, context)?;
    Ok(LclLlamaServerConfig {
        bin: optional_string(object, "bin", context)?.map(str::to_string),
        host: optional_string(object, "host", context)?.map(str::to_string),
        port: optional_u16(object, "port", context)?,
        ctx_size: optional_u32(object, "ctxSize", context)?,
        model_path: optional_string(object, "modelPath", context)?.map(str::to_string),
    })
}
...
mod tests {
    use super::{
        deep_merge_objects, parse_permission_mode_label, ConfigLoader, ConfigSource, LclConfig,
    ...
    #[test]
    fn default_config_home_prefers_lcl_config_home_env() {
        let _guard = crate::test_env_lock();
        let original_lcl = std::env::var_os("LCL_CONFIG_HOME");
        let original_claw = std::env::var_os("CLAW_CONFIG_HOME");
        std::env::set_var("LCL_CONFIG_HOME", "/tmp/lcl-config-home");
        std::env::set_var("CLAW_CONFIG_HOME", "/tmp/legacy-claw-config-home");

        let resolved = super::default_config_home();
        assert_eq!(resolved, PathBuf::from("/tmp/lcl-config-home"));

        match original_lcl {
            Some(value) => std::env::set_var("LCL_CONFIG_HOME", value),
            None => std::env::remove_var("LCL_CONFIG_HOME"),
        }
        match original_claw {
    ...
    #[test]
    fn parses_lcl_provider_and_llama_server_config() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        fs::create_dir_all(cwd.join(".claw")).expect("project config dir");
            ...
            r#"{
              ...
              "lcl": {
                ...
                "llamaServer": {
                  ...
                  "port": 8080,
                  "ctxSize": 8192,
                  "modelPath": "data/models/model.gguf"
                }
              }
            }"#,
        )
        ...
        let lcl = loaded.lcl();
        let provider = lcl.provider().expect("provider config should exist");
        assert!(provider.is_openai_compat());
        assert_eq!(provider.base_url.as_deref(), Some("http://127.0.0.1:8080/v1"));
        assert_eq!(provider.api_key.as_deref(), Some("local-dev-token"));

        let llama_server = lcl
            .llama_server()
            .expect("llama server config should exist");
        assert_eq!(llama_server.bin.as_deref(), Some("llama-server"));
        assert_eq!(llama_server.host.as_deref(), Some("127.0.0.1"));
        assert_eq!(llama_server.port, Some(8080));
        assert_eq!(llama_server.ctx_size, Some(8192));
        assert_eq!(llama_server.model_path.as_deref(), Some("data/models/model.gguf"));

        fs::remove_dir_all(root).expect("cleanup temp dir");
    }

    #[test]
    fn lcl_config_defaults_to_empty_when_unset() {
        let config = LclConfig::default();
        assert!(config.provider().is_none());
        assert!(config.llama_server().is_none());
    }
