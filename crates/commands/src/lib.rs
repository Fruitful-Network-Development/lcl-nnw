...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    if let Some(claw_config_home) = preferred_config_home_env() {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserClawConfigHome,
            claw_config_home.join(leaf),
        );
    }
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    if let Some(claw_config_home) = preferred_config_home_env() {
        return Ok(claw_config_home.join("skills"));
    }
    if let Ok(codex_home) = env::var("CODEX_HOME") {
    ...
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set LCL_CONFIG_HOME, CLAW_CONFIG_HOME, or HOME",
    ))
}

fn preferred_config_home_env() -> Option<PathBuf> {
    env::var_os("LCL_CONFIG_HOME")
        .or_else(|| env::var_os("CLAW_CONFIG_HOME"))
        .map(PathBuf::from)
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       lcl agents".to_string(),
        "  Sources          .claw/agents, ~/.claw/agents, $LCL_CONFIG_HOME/agents".to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "lcl agents [list|help]",
            "sources": [".claw/agents", "~/.claw/agents", "$LCL_CONFIG_HOME/agents"],
        },
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       lcl skills [list|install <path>|help|<skill> [args]]".to_string(),
        "  Invoke           /skills help overview -> $help overview".to_string(),
        "  Install root     $LCL_CONFIG_HOME/skills or ~/.claw/skills".to_string(),
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "aliases": ["/skill"],
            "direct_cli": "lcl skills [list|install <path>|help|<skill> [args]]",
            "invoke": "/skills help overview -> $help overview",
            "install_root": "$LCL_CONFIG_HOME/skills or ~/.claw/skills",
            "sources": [
                ".claw/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       lcl mcp [list|show <server>|help]".to_string(),
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "lcl mcp [list|show <server>|help]",
...
mod tests {
    ...
    fn renders_agents_reports_as_json() {
        ...
        assert_eq!(help["usage"]["direct_cli"], "lcl agents [list|help]");
    ...
    fn renders_skills_reports_as_json() {
        ...
        assert_eq!(
            ...
            "lcl skills [list|install <path>|help|<skill> [args]]"
        );
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       lcl agents"));
        assert!(agents_help
            .contains("Sources          .claw/agents, ~/.claw/agents, $LCL_CONFIG_HOME/agents"));
        ...
        assert!(skills_help.contains("Install root     $LCL_CONFIG_HOME/skills or ~/.claw/skills"));
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       lcl mcp [list|show <server>|help]"));
