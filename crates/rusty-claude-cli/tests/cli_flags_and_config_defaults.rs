...
fn status_command_applies_model_and_permission_mode_flags() {
    ...
    // when
    let output = Command::new(env!("CARGO_BIN_EXE_lcl"))
...
fn slash_command_names_match_known_commands_and_suggest_nearby_unknown_ones() {
    ...
    // when
    let help_output = Command::new(env!("CARGO_BIN_EXE_lcl"))
        .current_dir(&temp_dir)
        .arg("/help")
        .output()
        .expect("claw should launch");
    let unknown_output = Command::new(env!("CARGO_BIN_EXE_lcl"))
        .current_dir(&temp_dir)
        .arg("/zstats")
        .output()
        .expect("claw should launch");
...
fn local_subcommand_help_does_not_fall_through_to_runtime_or_provider_calls() {
    ...
    assert!(doctor_stdout.contains("Usage            lcl doctor"));
    ...
    assert!(status_stdout.contains("Usage            lcl status"));
...
fn command_in(cwd: &Path) -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_lcl"));
