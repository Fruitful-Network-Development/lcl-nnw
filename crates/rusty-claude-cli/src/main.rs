...
fn main() {
    if let Err(error) = run() {
        ...
        } else if message.contains("`lcl --help`") {
            eprintln!("error: {message}");
        } else {
            eprintln!(
                "error: {message}

Run `lcl --help` for usage."
            );
        }
        std::process::exit(1);
    }
    ...
    while index < args.len() {
        match args[index].as_str() {
                    ...
                    ) =>
            {
                ...
                // the arg to the API (e.g. `lcl prompt --help`) should show
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`lcl {command_name}` is a slash command. Use `lcl --resume SESSION.jsonl /{command_name}` or start `lcl` and run `/{command_name}`."
        )
    } else {
        format!(
            "`lcl {command_name}` is a slash command. Start `lcl` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn removed_auth_surface_error(command_name: &str) -> String {
    format!(
        "`lcl {command_name}` has been removed. Set OPENAI_API_KEY for local OpenAI-compatible runtimes or ANTHROPIC_API_KEY / ANTHROPIC_AUTH_TOKEN for Anthropic."
    )
}
...
fn parse_acp_args(args: &[String], output_format: CliOutputFormat) -> Result<CliAction, String> {
    match args {
        ...
        _ => Err(String::from(
            "unsupported ACP invocation. Use `lcl acp`, `lcl acp serve`, `lcl --acp`, or `lcl -acp`.",
        )),
    }
}
...
fn parse_direct_slash_cli_action(
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `lcl` and run it there, or use `lcl --resume SESSION.jsonl {command_name}` / `lcl --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `lcl --help` for CLI usage, or start `lcl` and use /help.");
    message
}
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:")
        .then_some(
            "Compatibility note: `/oh-my-claudecode:*` is a Claude Code/OMC plugin command. `lcl` does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
        )
}
...
/// Starts a minimal Model Context Protocol server that exposes lcl's
...
fn check_auth_health(config: Option<&runtime::RuntimeConfig>) -> DiagnosticCheck {
    ...
    let openai_api_key_present = env::var("OPENAI_API_KEY")
        .ok()
        .is_some_and(|value| !value.trim().is_empty());
    let config_openai_api_key_present = config
        .and_then(|runtime_config| runtime_config.lcl().provider())
        .and_then(|provider| provider.api_key.as_deref())
        .is_some_and(|value| !value.trim().is_empty());
    let env_details = format!(
        "Environment       anthropic_api_key={} anthropic_auth_token={} openai_api_key={} lcl_provider_api_key={}",
        ...
        if config_openai_api_key_present {
            "present"
        } else {
            "absent"
        },
    );
    let supported_auth_present = api_key_present
        || auth_token_present
        || openai_api_key_present
        || config_openai_api_key_present;
    ...
    match load_oauth_credentials() {
        ...
        .with_details(vec![
            ...
            "Suggested action  set OPENAI_API_KEY for local OpenAI-compatible runtimes or ANTHROPIC_API_KEY / ANTHROPIC_AUTH_TOKEN for Anthropic"
                .to_string(),
        ])
        .with_data(Map::from_iter([
            ...
            (
                "config_openai_api_key_present".to_string(),
                json!(config_openai_api_key_present),
            ),
...
fn check_install_source_health() -> DiagnosticCheck {
    ...
    .with_details(vec![
        ...
        "Recommended path  build from this repo or use the upstream binary documented in README.md"
            .to_string(),
        format!(
            "Deprecated crate  `{DEPRECATED_INSTALL_COMMAND}` installs a deprecated stub and does not provide the `lcl` binary"
        )
            .to_string(),
    ])
    .with_data(Map::from_iter([
...
const DUMP_MANIFESTS_OVERRIDE_HINT: &str =
    "Hint: set CLAUDE_CODE_UPSTREAM=/path/to/upstream or pass `lcl dump-manifests --manifests-dir /path/to/upstream`.";
    ...
    match command {
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `lcl` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
...
fn enforce_broad_cwd_policy(
    ...
    if is_interactive {
        ...
        eprintln!(
            "Warning: lcl is running from a very broad directory ({}).\n\
             The agent can read and search everything under this path.\n\
             Consider running from inside your project: cd /path/to/project && lcl",
            cwd.display()
        );
        ...
        let message = format!(
            "lcl is running from a very broad directory ({}). \
             The agent can read and search everything under this path. \
             Use --allow-broad-cwd to proceed anyway, \
             or run from inside your project: cd /path/to/project && lcl",
            cwd.display()
        );
        match output_format {
            CliOutputFormat::Json => {
 ...
 ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝\x1b[0m \x1b[38;5;208mCode\x1b[0m 🦞\n\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn print_mcp(
        ...
        // `lcl mcp serve` starts a stdio MCP server exposing lcl's built-in
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Usage            lcl status [--output-format <format>]
  Purpose          show the local workspace snapshot without entering the REPL
  Output           model, permissions, git state, config files, and sandbox status
  Formats          text (default), json
  Related          /status · lcl --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            lcl sandbox [--output-format <format>]
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Formats          text (default), json
  Related          /sandbox · lcl status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            lcl doctor [--output-format <format>]
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Formats          text (default), json
  Related          /doctor · lcl --resume latest /doctor"
            .to_string(),
        LocalHelpTopic::Acp => "ACP / Zed
  Usage            lcl acp [serve] [--output-format <format>]
  Aliases          lcl --acp · lcl -acp
  ...
  Formats          text (default), json
  Related          ROADMAP #64a (discoverability) · ROADMAP #76 (real ACP support) · lcl --help"
            .to_string(),
    }
}
...
fn print_acp_status(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let message = "ACP/Zed editor integration is not implemented in claw-code yet. `lcl acp serve` is only a discoverability alias today; it does not launch a daemon or Zed-specific protocol endpoint. Use the normal terminal surfaces for now and track ROADMAP #76 for real ACP support.";
    match output_format {
        CliOutputFormat::Text => {
            println!(
                "ACP / Zed\n  Status           discoverability only\n  Launch           `lcl acp serve` / `lcl --acp` / `lcl -acp` report status only; no editor daemon is available yet\n  Today            use `lcl prompt`, the REPL, or `lcl doctor` for local verification\n  Tracking         ROADMAP #76\n  Message          {message}"
            );
        }
        CliOutputFormat::Json => {
            println!(
...
fn build_runtime_with_plugin_state(
    ...
    let lcl_provider = feature_config.lcl().provider().cloned();
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        tool_registry: GlobalToolRegistry,
        progress_reporter: Option<InternalPromptProgressReporter>,
        lcl_provider: Option<runtime::LclProviderConfig>,
    ) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let client = match detect_provider_kind(&resolved_model) {
            ...
            ProviderKind::Xai | ProviderKind::OpenAi => {
                ...
                with_lcl_provider_env_overrides(lcl_provider.as_ref(), || {
...
fn with_lcl_provider_env_overrides<T, E>(
    lcl_provider: Option<&runtime::LclProviderConfig>,
    build: impl FnOnce() -> Result<T, E>,
) -> Result<T, E> {
    let should_override = lcl_provider.is_some_and(runtime::LclProviderConfig::is_openai_compat);
    if !should_override {
        return build();
    }

    let original_base_url = env::var_os("OPENAI_BASE_URL");
    let original_api_key = env::var_os("OPENAI_API_KEY");

    if let Some(base_url) = lcl_provider
        .and_then(|provider| provider.base_url.as_deref())
        .filter(|value| !value.trim().is_empty())
    {
        env::set_var("OPENAI_BASE_URL", base_url);
    }
    if let Some(api_key) = lcl_provider
        .and_then(|provider| provider.api_key.as_deref())
        .filter(|value| !value.trim().is_empty())
    {
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   lcl --resume {session_id} /compact"
    ));
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "lcl v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  lcl [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  lcl [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  lcl [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  lcl --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  lcl help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  lcl version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  lcl status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  lcl sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  lcl doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  lcl acp [serve]")?;
    writeln!(
        out,
    ...
    writeln!(out, "  lcl dump-manifests [--manifests-dir PATH]")?;
    writeln!(out, "  lcl bootstrap-plan")?;
    writeln!(out, "  lcl agents")?;
    writeln!(out, "  lcl mcp")?;
    writeln!(out, "  lcl skills")?;
    writeln!(out, "  lcl system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(out, "  lcl init")?;
    writeln!(
        out,
        "  lcl export [PATH] [--session SESSION] [--output PATH]"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  lcl --model local \"summarize this repo\"")?;
    writeln!(
        out,
        "  lcl --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(out, "  lcl --compact \"summarize Cargo.toml\" | wc -l")?;
    writeln!(
        out,
        "  lcl --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  lcl --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  lcl --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  lcl agents")?;
    writeln!(out, "  lcl mcp show my-server")?;
    writeln!(out, "  lcl /skills")?;
    writeln!(out, "  lcl doctor")?;
    ...
    writeln!(out, "  lcl init")?;
    writeln!(out, "  lcl export")?;
    writeln!(out, "  lcl export conversation.md")?;
    Ok(())
}
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   lcl --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("lcl --resume SESSION.jsonl /status"));
    }
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("lcl --help"));
    }
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("lcl help"));
        assert!(help.contains("lcl version"));
        assert!(help.contains("lcl status"));
        assert!(help.contains("lcl sandbox"));
        assert!(help.contains("lcl init"));
        assert!(help.contains("lcl acp [serve]"));
        assert!(help.contains("lcl agents"));
        assert!(help.contains("lcl mcp"));
        assert!(help.contains("lcl skills"));
        assert!(help.contains("lcl /skills"));
        assert!(help.contains("ultraworkers/claw-code"));
        assert!(help.contains("cargo install claw-code"));
        assert!(!help.contains("lcl login"));
        assert!(!help.contains("lcl logout"));
    }
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("lcl --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("lcl --resume latest"));
        assert!(help.contains("lcl --resume latest /status /diff /export notes.txt"));
    }
