...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let claw_json = cwd.join(".claw").join("settings.json");
    artifacts.push(InitArtifact {
        name: ".claw/settings.json",
        status: write_file_if_missing(&claw_json, STARTER_SETTINGS_JSON)?,
    });
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".claw/settings.json"));
